                           ...
                           ValaCodeContext* context)
{
	...
	if (!_tmp34_) {
		...
		if (_tmp38_) {
			...
			_tmp40_ = _tmp39_;
			_tmp37_ = !G_TYPE_CHECK_INSTANCE_TYPE (_tmp40_, VALA_TYPE_ARRAY_TYPE);
		} else {
			_tmp37_ = FALSE;
		}
		if (_tmp37_) {
		...
		if (vala_code_node_has_attribute_argument ((ValaCodeNode*) self, "CCode", "scope")) {
			...
			_tmp45_ = _tmp44_;
			_tmp43_ = G_TYPE_CHECK_INSTANCE_TYPE (_tmp45_, VALA_TYPE_DELEGATE_TYPE);
		} else {
			_tmp43_ = FALSE;
		}
		if (_tmp43_) {
	...
	if (_tmp63_ != NULL) {
		...
		_tmp67_ = _tmp66_;
		if (G_TYPE_CHECK_INSTANCE_TYPE (_tmp67_, VALA_TYPE_NULL_LITERAL)) {
		...
		if (_tmp64_) {
			...
			_tmp83_ = _tmp82_;
			if (!G_TYPE_CHECK_INSTANCE_TYPE (_tmp83_, VALA_TYPE_NULL_LITERAL)) {
	...
	_tmp144_ = _tmp143_;
	_tmp145_ = _vala_code_node_ref0 (G_TYPE_CHECK_INSTANCE_TYPE (_tmp144_, VALA_TYPE_METHOD) ? ((ValaMethod*) _tmp144_) : NULL);
	m = _tmp145_;
	_tmp146_ = m;
	if (_tmp146_ != NULL) {
		...
		if (_tmp158_) {
			...
			if (_tmp166_ >= 0) {
				...
				_tmp168_ = vala_callable_get_parameters ((ValaCallable*) _tmp167_);
				_tmp169_ = _tmp168_;
				_tmp170_ = index;
	...
	_tmp174_ = _tmp173_;
	_tmp175_ = old_source_file;
	vala_semantic_analyzer_set_current_source_file (_tmp174_, _tmp175_);
	_tmp176_ = vala_code_context_get_analyzer (context);
	_tmp177_ = _tmp176_;
	_tmp178_ = old_symbol;
	vala_semantic_analyzer_set_current_symbol (_tmp177_, _tmp178_);
	_tmp179_ = vala_code_node_get_error ((ValaCodeNode*) self);
	_tmp180_ = _tmp179_;
	result = !_tmp180_;
