                                        ...
                                        gint minor)
{
	...
	if (_tmp1_ > major) {
		...
		gint _tmp3_;
		_tmp3_ = self->priv->_target_glib_major;
		if (_tmp3_ == major) {
			gint _tmp4_;
			_tmp4_ = self->priv->_target_glib_minor;
			_tmp2_ = _tmp4_ >= minor;
		} else {
			_tmp2_ = FALSE;
		}
                                          ...
                                          const gchar* filename)
{
	...
	}
	{
		...
		_tmp4_ = _tmp3_ = g_strsplit (_tmp2_, "\n", 0);
		{
			...
			for (package_it = 0; package_it < _vala_array_length (_tmp3_); package_it = package_it + 1) {
				gchar* _tmp5_;
				gchar* package = NULL;
                                       ...
                                       gboolean cmdline)
{
	...
	if (_tmp3_) {
		...
		if (g_str_has_suffix (filename, ".vapi")) {
			...
			_tmp29_ = g_str_has_suffix (filename, ".gir");
		}
		if (_tmp29_) {
			ValaSourceFile* source_file = NULL;
			const gchar* _tmp30_;
			...
			_tmp30_ = rpath;
			_tmp31_ = vala_source_file_new (self, VALA_SOURCE_FILE_TYPE_PACKAGE, _tmp30_, NULL, cmdline);
			source_file = _tmp31_;
			_tmp32_ = source_file;
			vala_source_file_set_relative_filename (_tmp32_, filename);
			_tmp33_ = source_file;
			vala_code_context_add_source_file (self, _tmp33_);
			_vala_source_file_unref0 (source_file);
		} else {
			if (g_str_has_suffix (filename, ".c")) {
				const gchar* _tmp34_;
				_tmp34_ = rpath;
				vala_code_context_add_c_source_file (self, _tmp34_);
			} else {
				if (g_str_has_suffix (filename, ".h")) {
				} else {
					gchar* _tmp35_;
					gchar* _tmp36_;
					_tmp35_ = g_strdup_printf ("%s is not a supported source file type. Only .vala, .vapi, .gs, and .c" \
                          ...
                          ValaCodeVisitor* visitor)
{
	...
	while (TRUE) {
		...
		_tmp9_ = source_file;
		vala_source_file_accept (_tmp9_, visitor);
		_tmp10_ = index;
		index = _tmp10_ + 1;
		_vala_source_file_unref0 (source_file);
	}
...
vala_code_context_check (ValaCodeContext* self)
{
	...
	if (vala_report_get_errors (_tmp1_) > 0) {
		return;
	}
	_tmp2_ = self->priv->_analyzer;
	vala_semantic_analyzer_analyze (_tmp2_, self);
	_tmp3_ = self->priv->_report;
	if (vala_report_get_errors (_tmp3_) > 0) {
		return;
	}
	_tmp4_ = self->priv->_flow_analyzer;
	vala_flow_analyzer_analyze (_tmp4_, self);
	_tmp5_ = self->priv->_report;
	if (vala_report_get_errors (_tmp5_) > 0) {
		return;
	}
	_tmp6_ = self->priv->_used_attr;
	vala_used_attr_check_unused (_tmp6_, self);
}
                                 ...
                                 const gchar* pkg)
{
	...
	_tmp4_ = vala_code_context_get_file_path (self, _tmp1_, "vala" PACKAGE_SUFFIX "/vapi", "vala/vapi", _tmp3_, _tmp3__length1);
	_tmp5_ = _tmp4_;
	_g_free0 (_tmp1_);
	path = _tmp5_;
	_tmp6_ = path;
                                ...
                                const gchar* gir)
{
	...
	_tmp3_ = _tmp2_;
	_tmp3__length1 = _tmp2__length1;
	_tmp4_ = vala_code_context_get_file_path (self, _tmp1_, "gir-1.0", NULL, _tmp3_, _tmp3__length1);
	_tmp5_ = _tmp4_;
	_g_free0 (_tmp1_);
	result = _tmp5_;
	return result;
                                      ...
                                      const gchar* resource)
{
	...
	_tmp2_ = _tmp1_;
	_tmp2__length1 = 1;
	_tmp3_ = vala_code_context_get_file_path (self, resource, NULL, NULL, _tmp2_, 1);
	_tmp4_ = _tmp3_;
	...
	if (_tmp5_ == NULL) {
		...
		_tmp8_ = vala_code_context_get_file_path (self, resource, NULL, NULL, _tmp7_, _tmp7__length1);
		_g_free0 (filename);
		filename = _tmp8_;
	}
	result = filename;
                                     ...
                                     const gchar* gir_filename)
{
	...
	_tmp14_ = vala_code_context_get_file_path (self, _tmp11_, NULL, NULL, _tmp13_, _tmp13__length1);
                                      ...
                                      const gchar* filename)
{
	...
	fprintf (_tmp4_, "%s:", filename);
	{
		...
		while (TRUE) {
			...
			src = (ValaSourceFile*) _tmp15_;
			_tmp17_ = src;
			_tmp18_ = vala_source_file_get_file_type (_tmp17_);
			_tmp19_ = _tmp18_;
			if (_tmp19_ == VALA_SOURCE_FILE_TYPE_FAST) {
				ValaSourceFile* _tmp20_;
				gboolean _tmp21_;
				gboolean _tmp22_;
				_tmp20_ = src;
				_tmp21_ = vala_source_file_get_used (_tmp20_);
				_tmp22_ = _tmp21_;
				_tmp16_ = _tmp22_;
			} else {
				_tmp16_ = FALSE;
			...
			if (_tmp16_) {
				...
				_tmp23_ = stream;
				_tmp24_ = src;
				_tmp25_ = vala_source_file_get_filename (_tmp24_);
				_tmp26_ = _tmp25_;
...
vala_code_context_realpath (const gchar* name)
{
			...
			if (_tmp23_ == ((glong) 0)) {
				...
				if (_tmp24_) {
					...
					if (_tmp27_) {
						...
						if (((glong) _tmp32_) > _tmp33_) {
							{
								...
								while (TRUE) {
									...
									if (!_tmp34_) {
										const gchar* _tmp35_;
										_tmp35_ = rpath;
										if (!(!vala_code_context_ends_with_dir_separator (_tmp35_))) {
											break;
										}
									}
									_tmp34_ = FALSE;
						...
						gchar* _tmp50_;
						_tmp41_ = rpath;
						if (!vala_code_context_ends_with_dir_separator (_tmp41_)) {
							const gchar* _tmp42_;
							gchar* _tmp43_;
							_tmp42_ = rpath;
						...
						_tmp46_ = end;
						_tmp47_ = start;
						_tmp48_ = string_substring (_tmp45_, (glong) 0, (glong) (((gchar*) _tmp46_) - ((gchar*) _tmp47_)));
						_tmp49_ = _tmp48_;
	...
	_tmp54_ = _tmp53_;
	_tmp55_ = root_len;
	if (((glong) _tmp54_) > _tmp55_) {
		const gchar* _tmp56_;
		_tmp56_ = rpath;
		_tmp51_ = vala_code_context_ends_with_dir_separator (_tmp56_);
	} else {
		_tmp51_ = FALSE;
	}
	if (_tmp51_) {
	...
	if (G_DIR_SEPARATOR != '/') {
		...
		components = _tmp64_;
		components_length1 = _vala_array_length (_tmp63_);
		_components_size_ = components_length1;
		_tmp65_ = components;
		_tmp65__length1 = components_length1;
                                     ...
                                     const gchar* package_name)
{
	...
	{
		gint _tmp5_ = 0;
		g_spawn_command_line_sync (pc, NULL, NULL, &_tmp5_, &_inner_error_);
		exit_status = _tmp5_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
			gboolean _tmp6_ = FALSE;
                                         ...
                                         const gchar* package_name)
{
	...
	output = NULL;
	{
		gchar* _tmp5_ = NULL;
		gint _tmp6_ = 0;
		g_spawn_command_line_sync (pc, &_tmp5_, NULL, &_tmp6_, &_inner_error_);
		_g_free0 (output);
		output = _tmp5_;
		exit_status = _tmp6_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
                                            ...
                                            const gchar* package_name)
{
	...
	output = NULL;
	{
		...
		gint _tmp10_;
		_tmp7_ = pc;
		g_spawn_command_line_sync (_tmp7_, &_tmp8_, NULL, &_tmp9_, &_inner_error_);
		_g_free0 (output);
		output = _tmp8_;
		exit_status = _tmp9_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
		...
		if (_tmp10_ != 0) {
			...
			gchar* _tmp13_;
			gchar* _tmp14_;
			_tmp11_ = self->priv->_pkg_config_command;
			_tmp12_ = exit_status;
			_tmp13_ = g_strdup_printf ("%s exited with status %d", _tmp11_, _tmp12_);
			_tmp14_ = _tmp13_;
...
vala_code_context_get_abi_stability (ValaCodeContext* self)
{
	...
	gboolean _tmp0_;
	g_return_val_if_fail (self != NULL, FALSE);
	_tmp0_ = self->priv->_abi_stability;
	result = _tmp0_;
	return result;
}
...
gint
vala_code_context_get_target_glib_major (ValaCodeContext* self)
{
	gint result;
	gint _tmp0_;
	g_return_val_if_fail (self != NULL, 0);
...
void
vala_code_context_set_target_glib_major (ValaCodeContext* self,
                                         gint value)
{
	g_return_if_fail (self != NULL);
...
gint
vala_code_context_get_target_glib_minor (ValaCodeContext* self)
{
	gint result;
	gint _tmp0_;
	g_return_val_if_fail (self != NULL, 0);
...
void
vala_code_context_set_target_glib_minor (ValaCodeContext* self,
                                         gint value)
{
	g_return_if_fail (self != NULL);
