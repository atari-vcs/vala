                             ...
                             gconstpointer item)
{
	...
	_tmp5_ = ((item != NULL) && (self->priv->g_dup_func != NULL)) ? self->priv->g_dup_func ((gpointer) item) : ((gpointer) item);
		...
		index = 0;
		{
			...
			while (TRUE) {
				...
				if (_tmp4_ (_tmp7_, item)) {
					...
					gpointer _tmp10_;
					_tmp8_ = index;
					_tmp9_ = vala_list_remove_at ((ValaList*) self, _tmp8_);
					_tmp10_ = _tmp9_;
                                ...
                                gint index)
{
	...
	_tmp5_ = self->_items;
	_tmp5__length1 = self->_items_length1;
	((_tmp5_[index] == NULL) || (self->priv->g_destroy_func == NULL)) ? NULL : (_tmp5_[index] = (self->priv->g_destroy_func (_tmp5_[index]), NULL));
	_tmp5_[index] = NULL;
                       ...
                       gint delta)
{
	...
	_tmp3_ = self->_size;
	_vala_array_move (self->_items, sizeof (gpointer), start, start + delta, _tmp3_ - start);
	_tmp4_ = self->_size;
	self->_size = _tmp4_ + delta;
}
                                ...
                                gint new_count)
{
	...
	if (_tmp1_ > _tmp2__length1) {
		...
		if (new_count > _tmp4__length1) {
			...
			_tmp3_ = 2 * _tmp6__length1;
		}
		vala_array_list_set_capacity (self, _tmp3_);
	}
}
                              ...
                              gint value)
{
	...
	g_return_if_fail (self != NULL);
	_tmp0_ = self->_size;
	_vala_assert (value >= _tmp0_, "value >= _size");
	_tmp1_ = value;
	self->_items = g_renew (gpointer, self->_items, value);
	(_tmp1_ > self->_items_length1) ? memset (self->_items + self->_items_length1, 0, sizeof (gpointer) * (_tmp1_ - self->_items_length1)) : NULL;
	self->_items_length1 = _tmp1_;
	self->__items_size_ = _tmp1_;
}
...
vala_array_list_iterator_real_remove (ValaIterator* base)
{
	...
	_tmp9_ = self->priv->_list;
	_tmp10_ = self->priv->_index;
	_tmp11_ = vala_list_remove_at ((ValaList*) _tmp9_, _tmp10_);
	_tmp12_ = _tmp11_;
